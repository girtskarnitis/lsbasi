...
class ProcessedPath(threading.Thread):
...
class NodeVisitor(threading.Thread):
    def visit(self, node):
...
class Interpreter(NodeVisitor):
    ...
    def visit_tCompound(self, node):
        ...
        ExecPath.addClosPar()
        if _SHOULD_LOG_ALL:
            print(f'visit_transaction end:{tdata.npk}')              
        t[tdata.npk].clear()
        tm.set()
        t[tdata.npk].wait()        
    ...
    def visit_ProcedureCall1(self, npk):
        tdata.npk=npk
        while True:
            if _SHOULD_LOG_ALL:            
                print(f'Visit prodecure call gaidu t[{npk}]')
            t[npk].wait()
            t[npk].clear()
            if _SHOULD_LOG_ALL:            
                print(f'Sagaidīju t[{npk}]')

            if _SHOULD_LOG_ALL:
                print(f'Enter1 visit_ProcedureCall1, npk:{npk}')
            ...
            active[npk] = 0
            if _SHOULD_LOG_ALL:
                print(f'visit_ProcedureCall1, active[{npk}]=0')
            t[npk].clear()
            tm.set()
    ...
    def visit_ConcurrentCall(self, node):
        ...
        if _SHOULD_LOG_ALL:
            print(f'concurnodes size:{len(concurnode)}')        

        concurnode[1]=node.left
        concurnode[2]=node.right
        while active[1]==1 or active[2]==1:
            if _SHOULD_LOG_ALL:
                print(f'active[1]=={active[1]}, active[2]=={active[2]}')            
            if active[1]==1 and active[2]==1:
                    if ProcPath.nextIf():
                        if _SHOULD_LOG_ALL:
                            print(f'Enter visit_ConcurrentCall True Path')            
                        tm.clear()
                        if _SHOULD_LOG_ALL:
                            print(f'Enter visit_ConcurrentCall True first procedure')        
                        t[1].set()
                        if _SHOULD_LOG_ALL:                        
                            print('visit_ConcurrentCall gaidu tm pirmajai procedūrai 1')
                        tm.wait()
                        tm.clear()
                        if _SHOULD_LOG_ALL:                        
                            print('Sagaidīju visit_ConcurrentCall tm pirmajai procedūrai 1')            
                    else:
                        if _SHOULD_LOG_ALL:
                            print(f'Enter visit_ConcurrentCall Flase Path')  
                        tm.clear()
                        if _SHOULD_LOG_ALL:
                            print(f'Enter visit_ConcurrentCall False first procedure') 
                        t[2].set()
                        if _SHOULD_LOG_ALL:                        
                            print('visit_ConcurrentCall gaidu tm otrajai procedūrai 2')
                        tm.wait()
                        tm.clear()
                        if _SHOULD_LOG_ALL:                        
                            print('Sagaidīju visit_ConcurrentCall tm otrajai procedūrai 2')            
            elif active[1]==1:
                if _SHOULD_LOG_ALL:
                    print(f'Enter visit_ConcurrentCall Pirmais Path')            
                tm.clear()
                if _SHOULD_LOG_ALL:
                    print(f'Enter visit_ConcurrentCall Pirmais first procedure')        
                t[1].set()
                if _SHOULD_LOG_ALL:                
                    print('visit_ConcurrentCall gaidu tm Pirmais pirmajai procedūrai 1')
                tm.wait()
                tm.clear()
                if _SHOULD_LOG_ALL:                
                    print('Sagaidīju visit_ConcurrentCall tm Pirmais pirmajai procedūrai 1')            
            elif active[2]==1: 
                if _SHOULD_LOG_ALL:
                    print(f'Enter visit_ConcurrentCall Otraos Path')  
                tm.clear()
                if _SHOULD_LOG_ALL:
                    print(f'Enter visit_ConcurrentCall Otrais second procedure') 
                t[2].set()
                if _SHOULD_LOG_ALL:                
                    print('visit_ConcurrentCall gaidu tm Otrais otrajai procedūrai 2')
                tm.wait()
                tm.clear()
                if _SHOULD_LOG_ALL:                
                    print('Sagaidīju visit_ConcurrentCall tm Otrais otrajai procedūrai 2')            
    ...
    def interpret(self):
        while True:
            if _SHOULD_LOG_ALL:            
                print('interpret gaidu tm')
            tm.wait()
            tm.clear()
            if _SHOULD_LOG_ALL:            
                print('interpet sagaidīju tm')
            tree = self.tree
            if tree is None:
                tm.clear
                tmm.set()
...
def main():
    ...
    global callstack
    # global concurnodeLeft, concurnodeRight
    # global tm, tmm, t
    # global jm, j1, j2
    ...
    je = threading.Thread(target=ProcPath.run)
    jm = threading.Thread(target=interpreter.interpret)
    j1 = threading.Thread(target=interpreter.visit_ProcedureCall1, args=(1,))
    j2 = threading.Thread(target=interpreter.visit_ProcedureCall1, args=(2,))
    jm.start()
    j1.start()
    j2.start()
    while not(ProcPath.isDone()):
        ...
        if _SHOULD_LOG_ALL:        
            print('Main gaidu tmm')
        tmm.wait()
        if _SHOULD_LOG_ALL:        
            print('Main sagaidīju tmm')
